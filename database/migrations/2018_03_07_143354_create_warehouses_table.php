    ...
    public function up()
    {
        Schema::create('device_warehouses', function (Blueprint $table) {
           ...
           // $table->string('device_ipaddr',15)->nullable()->comment('设备IP');//机器ip
            ...
            $table->double('device_price')->default(0)->comment('购买价格');//机器mac地址
           ...
           // $table->tinyInteger('equipment_use_id')->default(0)->comment('设备用途id');//软件配置情况price
            ...
            $table->date('start_date')->nullable()->comment('维保起始日');//维保开始日
            $table->date('over_date')->nullable()->comment('维保截止日');//维保到期日

            $table->tinyInteger('device_work_state')->comment('当前设备工作状态');//当前设备工作状态  待修理  0正常 1待修 2淘汰 3报废 （//在设备登记时建立 设备状态登记表）
            $table->tinyInteger('device_save_state')->comment('当前设备保存状态');//当前设备保存状态 0入库、1 调拨出 至某机构 2 从某机构 调拨入  3 借用、机构确认调入(增加信息) 3 清理  4出库(移出本系统)//(设备出入登记表)
            ...
            //$table->integer('institution_id')->default(0)->comment('设备所在机构');//所在机构id  登记入库时此 机构号为0
            //$table->string('group_num')->default('0-0')->comment('自助设备所属柜组(设备号-虚拟柜员号)');//所属柜组自助设备必填  或者 设备号-虚拟柜员号
            //布局类型
            //$table->integer('layout_num')->default(0)->comment('自助设备布局类型');//所在机构id  登记入库时此 机构号为0
            //启用日期
